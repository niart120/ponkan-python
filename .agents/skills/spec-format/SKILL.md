...
- テストリストの状態は `todo` / `red` / `green` / `refactor-done` / `deferred` を基本にする。
...
4. TDD で進める作業では、振る舞い仕様と TDD Test List を作る。
5. 対象ファイルの変更種別 (新規 / 修正 / 削除) を明記する。
6. チェックリストに実装タスクと検証コマンドを列挙する。
7. 実装後、実行結果を反映してチェックリストを更新する。
