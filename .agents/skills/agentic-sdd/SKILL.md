...
3. 変更を伴う作業では現在 branch、default branch、`git status --short` を確認する。
4. ユーザ入力から Intent Delta を抽出する。差分がなければ `none` とする。
5. 現在の worktree と既存テストを確認し、実装済み範囲を事実として扱う。
6. 次の Work Unit を 1つだけ選ぶ。
...
変更を伴う Work Unit では、Plan 前に git context を確認する。

| 状態 | 扱い |
| ---- | ---- |
| read-only 調査 | branch 作成は不要。 |
| 作業ブランチ上で clean | そのまま進める。 |
...
変更を伴う Work Unit は project の GitHub Flow に従い、短命な作業ブランチから PR 経由で default branch に取り込む。
