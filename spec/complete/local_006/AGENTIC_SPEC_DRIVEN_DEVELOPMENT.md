...
| branch 境界 | 変更開始時に個別判断 | 変更を伴う Work Unit は作業ブランチ上で開始する |
...
| ファイル | 変更種別 | 変更内容 |
| -------- | -------- | -------- |
| `spec/complete/local_006/AGENTIC_SPEC_DRIVEN_DEVELOPMENT.md` | 新規 | Agentic SDD 全体像を定義する。 |
| `spec/complete/local_006/AGENT_BOOTSTRAP_PLAYBOOK.md` | 新規 | ユーザと Main Agent の開始手順を定義する。 |
| `spec/complete/local_006/QUALITY_GATES_AND_REVIEW.md` | 新規 | review を品質ゲートとして再定義する。 |
| `spec/complete/local_006/RESEARCH_NOTES.md` | 新規 | 調査した外部手法と採用判断を記録する。 |
...
| Git context を確認する | 変更を伴う Work Unit を開始する | 現在 branch、default branch、dirty 状態を確認し、必要なら作業ブランチを作る | TCR は隔離 worktree |
...
| 状態 | テスト項目 | 種別 | 関連仕様 | 備考 |
| ---- | ---------- | ---- | -------- | ---- |
| green | `agentic-sdd` skill が Spec -> Plan -> Tasks -> Implement を案内する | new behavior | 3.1 | `quick_validate.py .agents/skills/agentic-sdd` 通過 |
| green | Intent Delta だけで bootstrap できる prompt を `AGENTS.md` に追加する | new behavior | 3.1 | Agentic SDD 節へ追記 |
| green | Work Unit 選択時に未選択 Step を実装しない rule を明記する | regression | 3.1 | `AGENTS.md` と `agentic-sdd` skill に明記 |
| green | task graph で Subagent 分担可否を判定し、必要に応じて sidecar task を起動する | new behavior | 3.1 | `AGENTS.md` と `agentic-sdd` skill に明記 |
| green | gate 失敗時に spec / plan / task へ戻る loop を定義する | new behavior | 3.1 | `agentic-sdd` と `agentic-self-review` skill に反映 |
| green | Git Context Gate で default branch 上の変更開始を避ける | regression | 3.1 | `local_007` と skill に反映 |
...
### 4.1 Git Context Gate

Main Agent は、変更を伴う Work Unit を始める前に次を確認する。

| 状態 | 扱い |
| ---- | ---- |
| read-only 調査 | branch 作成は不要。 |
| 作業ブランチ上で clean | そのまま Plan へ進む。 |
| default branch 上で clean | 作業内容に応じた branch を作るか、既存作業ブランチへ切り替える。 |
| dirty worktree | 既存変更を読んで、ユーザ変更を破棄せず作業可否を判断する。 |
| TCR 明示 | `tcr-workflow-exp` の隔離 worktree を使う。 |

通常の TDD / Agentic SDD は隔離 worktree を要求しない。隔離 worktree は TCR の失敗時 revert 境界としてだけ使う。
...
### 4.3 Subagent Auto-Orchestration
...
```text
- selected Work Unit
- git context: branch / dirty status / worktree policy
- Intent Delta の有無
- 実装したこと / しなかったこと
- 実行した gate と結果
- 原典 audit の有無
...
- [x] `agentic-sdd` skill を追加する。
- [x] `agentic-self-review` skill を Quality Gate 報告用として追加する。
- [x] `AGENTS.md` に Intent Delta、Work Unit bootstrap、Subagent auto-orchestration を追記する。
- [x] `.github/PULL_REQUEST_TEMPLATE.md` に Agentic SDD gate 結果欄を追加する。
- [x] `.codex` rules / hook に実機系 command の承認境界を追加する。
- [x] skill と hook の検証を実行する。
- [x] レビュー完了。

検証結果:

```text
PYTHONUTF8=1 uv run python C:\Users\train\.codex\skills\.system\skill-creator\scripts\quick_validate.py .agents\skills\agentic-sdd -> Skill is valid!
PYTHONUTF8=1 uv run python C:\Users\train\.codex\skills\.system\skill-creator\scripts\quick_validate.py .agents\skills\agentic-self-review -> Skill is valid!
uv run python -c "<hook policy representative cases>" -> hook policy cases passed
