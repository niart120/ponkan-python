...
| `.agents/skills/tcr-lab/SKILL.md` | 新規 | 任意の実験用 TCR skill を追加し、通常 workflow からは外す。 |
...
`tcr-lab` は通常の開発 workflow には組み込まない。現在の Codex policy は `git reset --hard` などの破壊的 git 操作を禁止しているため、TCR は安全な一時 branch / worktree と明示承認がある場合に限る。
