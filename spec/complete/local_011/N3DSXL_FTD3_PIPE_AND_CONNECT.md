...
- [x] `cc3dsfs-source-audit` で command payload、timeout、connect sequence の参照元を記録済み。
...
| 項目 | 内容 |
| ---- | ---- |
| 配置 | `spec/complete/local_011/N3DSXL_FTD3_PIPE_AND_CONNECT.md` |
...
| 次 Work Unit | `spec/complete/local_012/N3DSXL_RAW_CAPTURE_FIXTURE_AND_DECODER.md` |
...
| 状態 | テスト項目 | 種別 | 関連仕様 | 備考 |
| ---- | ---------- | ---- | -------- | ---- |
| green | create pipe payload が source audit fixture と一致する | characterization | 3.1 | 原典 audit 後に fixture 化 |
| green | abort pipe payload が source audit fixture と一致する | characterization | 3.1 | pipe id parameter |
| green | set stream pipe payload に 2D capture size が入る | characterization | 3.1 | `capture_size(False)` |
| green | read_pipe は prepare read 後に bulk IN を呼ぶ | new behavior | 3.1 | fake backend |
| green | write_pipe は prepare write 後に bulk OUT を呼ぶ | new behavior | 3.1 | fake backend |
| green | Candidate 由来でない session は command 送信前に拒否される | safety | 3.1 | fake session |
| green | connect sequence が source audit の順序で pipe methods を呼ぶ | characterization | 3.1 | fake pipe |
| green | connect 失敗時に command name と libusb status が error に残る | regression | 3.1 | debug |
| deferred | 実機 create/abort が libusb error なしで返る | hardware | 3.1 | `requires_n3dsxl`。人間承認待ち |
| deferred | 実機 2D connect が完了する | hardware | 3.1 | `requires_n3dsxl`。人間承認待ち |
...
| 項目 | 参照元候補 | 実装前状態 |
| ---- | ---------- | ---------- |
| create / destroy pipe command | `source/CaptureDeviceSpecific/3DSCapture_FTD3/3dscapture_ftd3_libusb_comms.cpp` | command id `3`、create pipe id `0x82`、payload size 20 bytes を確認 |
| abort pipe command | 同上 | command id `0`、abort -> abort -> destroy の順序を確認 |
| read/write prepare command | 同上 | command id `1`、length little-endian、prepare 後に bulk IN/OUT を行うことを確認 |
| set stream pipe command | 同上 | command id `2`、`BULK_IN 0x82` と capture size を渡すことを確認 |
| connect sequence | `3dscapture_ftd3_shared.cpp` | drain、preemptive reset、SPI/config、firmware、3D config、set_stream/abort/set_stream の順序を確認。preemptive reconnect は hardware gate で検証 |
| timeouts | `3dscapture_ftd3_libusb_comms.cpp`, `3dscapture_ftd3_shared.cpp` | command timeout 500ms、N3DSXL config wait 200ms を確認 |
...
| 項目 | Evidence |
| ---- | -------- |
| source-audit complete | `3dscapture_ftd3_libusb_comms.cpp` と `3dscapture_ftd3_shared.cpp` を確認し、payload fixture と sequence test に反映 |
| local complete | `uv run pytest tests\unit\test_ftd3_pipe_payloads.py tests\unit\test_n3dsxl_connect_sequence.py -q` が 11 passed |
| unit regression | `uv run pytest tests\unit -q` が 27 passed |
| static | `uv run ruff format --check .`、`uv run ruff check src tests`、`uv run ty check --no-progress` が pass |
| hardware pending | `uv run pytest tests\e2e\test_n3dsxl_ftd3_pipe.py tests\e2e\test_n3dsxl_connect.py -q` は `PONKAN_RUN_N3DSXL` 未設定で skip。実機 command は未実行 |
...
- [x] `cc3dsfs-source-audit` で FTD3 command payload と connect sequence を記録する。
- [x] payload builder の characterization test を書く。
- [x] `Ftd3Pipe` の fake backend unit test を書く。
- [x] `Ftd3Pipe` を実装する。
- [x] `N3DSXLProtocol.connect(mode_3d=False)` の fake sequence test を書く。
- [x] connect sequence を実装する。
