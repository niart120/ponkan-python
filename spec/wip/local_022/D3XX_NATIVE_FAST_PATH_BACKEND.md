...
- [ ] Direct DLL の handle / function surface を private PyD3XX internals に依存するか、明示的な native binding として分離するかを実装前に決める。
- [ ] Overlapped structure と cancellation API の実際の calling convention を FTDI D3XX Programmer's Guide と installed header / DLL で再確認する。

### 1.6 Work Unit メタデータ

| 項目 | 内容 |
| ---- | ---- |
| 配置 | `spec/wip/local_022/D3XX_NATIVE_FAST_PATH_BACKEND.md` |
| 対応 Step | Step 7-8 follow-up: opt-in D3XX native fast path backend design |
| 前提 Work Unit | `local_013`、`local_016`、`local_021` |
| local task | native backend design、unit test list、source audit item、safety gate の固定 |
| hardware task | 実装後に 10 秒 timing smoke、60 秒 performance smoke、sequential baseline 比較。承認後のみ実行 |
| 選択条件 | `local_021` の low latency default よりさらに copy / wakeup / host latency を減らす必要が出たとき |
| 完了証拠 | この仕様、Work Unit index、実装前 gate の明示 |

## 2. 対象ファイル

| ファイル | 変更種別 | 変更内容 |
| -------- | -------- | -------- |
| `spec/wip/local_022/D3XX_NATIVE_FAST_PATH_BACKEND.md` | 新規 | D3XX native fast path backend の設計、非対象、test list、hardware gate を定義する。 |
| `spec/complete/local_008/WORK_UNIT_INDEX.md` | 修正 | follow-up Work Unit として `local_022` を索引に追加し、`local_021` の実機完了状態を反映する。 |
| `src/py3dscapture/transport/d3xx_native_streaming.py` | 新規予定 | opt-in native fast path backend。今回の設計 Work Unit では実装しない。 |
| `src/py3dscapture/transport/d3xx_native.py` | 新規予定 | D3XX DLL function surface、native handle、overlapped helper。今回の設計 Work Unit では実装しない。 |
| `tests/unit/test_d3xx_native_streaming_backend.py` | 新規予定 | fake native API による submit / completion / cancel / lifetime test。今回の設計 Work Unit では実装しない。 |
| `tests/performance/test_n3dsxl_streaming_smoke.py` | 修正予定 | 実装後、backend selector で native backend smoke を追加する。今回の設計 Work Unit では実装しない。 |
...
| native DLL surface がない | PyD3XX private `_DLL` / `_Handle` または新 binding が使えない | silent fallback せず、native backend unavailable として失敗する | 測定結果の混同防止 |
...
| 状態 | テスト項目 | 種別 | 関連仕様 | 備考 |
| ---- | ---------- | ---- | -------- | ---- |
| todo | native backend 指定時に DLL surface がなければ config error になる | behavior | 3.1 | silent fallback しない |
| todo | preallocated native buffer owner が completion まで buffer と overlapped state を保持する | safety | 3.1 | fake native API が owner lifetime を検査 |
| todo | submit は read ごとの native buffer allocation を行わない | performance contract | 1.4 | fake allocator count |
| todo | completion pump は callback へ `transferred` / `status` / timestamps だけを渡す | behavior | 3.1 | decode は engine 側 |
| todo | short transfer は既存 engine で decode されず drop counter になる | regression | 3.1 | fake native backend + engine |
| todo | native error は decode されず `usb_errors` になる | regression | 3.1 | fake status |
| todo | cancel 後の新規 submit は拒否される | shutdown | 3.1 | race guard |
| todo | release は pending overlapped の drain 前に native resources を解放しない | safety | 3.1 | fake call order |
| todo | timing summary が sequential backend と同じ keys を持つ | compatibility | 3.1 | `local_021` schema |
| deferred | 実機 D3XX native backend 10 秒 timing smoke を実行する | hardware | 5 | 承認後のみ |
| deferred | 実機 D3XX native backend 60 秒 performance smoke を実行する | hardware | 5 | sequential low-latency baseline と比較 |
...
| D3XX API surface | `https://ftdichip.com/wp-content/uploads/2020/08/AN_379-D3xx-Programmers-Guide.pdf` | `FT_ReadPipeEx`、`FT_GetOverlappedResult`、`FT_InitializeOverlapped`、`FT_ReleaseOverlapped`、`FT_SetStreamPipe`、`FT_AbortPipe` の仕様を実装前に固定する。 |
...
drain()
  - wait until no NativeReadSlot is in_flight
...
uv run pytest -m "requires_n3dsxl and performance" tests/performance/test_n3dsxl_streaming_smoke.py -q
```
...
| `submit_to_complete_ms.p95/p99` | sequential low-latency baseline より悪化しない |
...
- [ ] 実装 Work Unit で `D3xxNativeApi` fake test を作成する。
- [ ] 実装 Work Unit で opt-in native backend を作成する。
- [ ] 実装 Work Unit で実機 timing smoke を実行する。
- [ ] 実装 Work Unit で sequential low-latency baseline と比較する。

## 7. Design Decision

Native fast path は採用候補として残すが、現時点で default へ入れる理由はない。`local_021` の低遅延 default は 60 秒実機 smoke で安定し、host pipeline latency は約 2 frame 規模まで下がっている。

次に実装する場合は、まず native API wrapper と fake native lifecycle test を作り、`FT_ReadPipeEx` / overlapped / cancel / release の境界を unit test で固定する。最初の hardware smoke は Phase 1 の backend-owned native buffer で行い、direct `RawFrameSlot.buffer` write は Phase 2 として分離する。
