...
| ファイル | 変更種別 | 変更内容 |
| -------- | -------- | -------- |
| `spec/wip/local_023/HIGH_LEVEL_CAPTURE_READ_API.md` | 新規 | `read()` と `CaptureOutput` 中心の高レベル API 仕様を追加する。 |
...
| `src/py3dscapture/errors.py` | 修正 | 必要なら `DeviceNotFound` を追加する。timeout は recoverable state として `None` を返すため例外にしない。 |
...
| `tests/e2e/test_n3dsxl_high_level_capture_read_api.py` | 新規 | 実機 new 3DS XL で high-level API から 1 frame 以上取得できることを検証する。 |
...
| 状態 | テスト項目 | 種別 | 関連仕様 | 備考 |
| ---- | ---------- | ---- | -------- | ---- |
| todo | `open_capture()` は fake opener で `CaptureReader` を返す | new behavior | 3.1 | 実機なし |
| todo | `CaptureReader.read_frame()` は fake engine の `CaptureFrame` を返す | new behavior | 3.1 | domain API |
| todo | `CaptureReader.read()` は default で両画面縦並べ ndarray を返す | new behavior | 3.1 | 主要 API |
| todo | frame がない場合、`read()` / `read_frame()` は timeout 後 `None` を返す | edge | 3.1 | recoverable state |
| todo | `read()` / `read_frame()` は queued frames の最新 frame を使う | behavior | 3.1 | latency 回避 |
| todo | `read(output=CaptureOutput.TOP)` は top ndarray を返す | new behavior | 3.1 | shape `(240, 400, 3)` |
| todo | `read(output=CaptureOutput.BOTTOM)` は bottom ndarray を返す | new behavior | 3.1 | shape `(240, 320, 3)` |
| todo | `read(output=CaptureOutput.BOTH_VERTICAL)` は縦並べ ndarray を返す | new behavior | 3.1 | shape `(480, 400, 3)` |
| todo | `read(colorspace="BGR")` は channel order を変換した copy を返す | compatibility | 3.1 | original frame は RGB のまま |
| todo | invalid output は `ValueError` | edge | 3.1 | enum validation |
| todo | backend hard error は `CaptureError` 系を raise する | error | 3.1 | error を握りつぶさない |
| todo | context manager exit は `close()` を呼ぶ | cleanup | 3.1 | idempotent |
| todo | `close()` は engine `stop()` と backend release を 1 回だけ実行する | cleanup | 3.1 | double close |
| todo | `mode_3d=True` は `UnsupportedOperation` | scope | 3.1 | MVP 外 |
| todo | explicit `d3xx-native` は unavailable 時に fallback しない | safety | 3.1 | opt-in backend |
| todo | OpenCV property API を public surface に追加しない | design guard | 3.1 | `get` / `set` / `CAP_PROP_*` 非対象 |
| todo | 実機 high-level API で 1 frame 以上取得できる | hardware | 5 | `requires_n3dsxl` |
...
- [ ] `CaptureOutput` enum を実装する。
- [ ] `CaptureConfig` と validation を実装する。
- [ ] `CaptureReader` と `open_capture()` を実装する。
- [ ] fake engine を使って `read_frame()` の TDD item を green にする。
- [ ] timeout / latest frame / error propagation の TDD item を green にする。
- [ ] `read()` の output / colorspace TDD item を green にする。
- [ ] context manager と idempotent close の TDD item を green にする。
- [ ] OpenCV property API を追加していないことを design guard で確認する。
- [ ] high-level opener を実装し、既存 N3DSXL safety boundary を通す。
- [ ] `__init__.py` の public export を更新する。
- [ ] README に `open_capture()` / `CaptureOutput` / `read()` の最小使用例を追加する。
- [ ] local gate を実行する。
- [ ] 承認後に実機 high-level E2E を実行する。
- [ ] gate 結果をこの仕様と `WORK_UNIT_INDEX.md` に反映し、complete へ移動する。
