...
from ponkan.transport.d3xx_native_streaming import D3xxNativeFastPathBackend
...
def test_n3dsxl_d3xx_native_streaming_60_second_performance_smoke(tmp_path: Path) -> None:
    ...
    stats_path = (
        n3dsxl_artifact_dir("native-performance-smoke", root=tmp_path) / "stream_stats.json"
    )
    ...
    assert "completion_interval_ms" in stats.timing
